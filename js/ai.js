...
const AI = {
  ...
  /**
   * Generates a full deck outline grounded in real PubMed citations.
   * Returns parsed JSON: { title, slides:[{title,bullets,layout,imageQuery,notes}] }
   */
  async generateDeck({ topic, level, slideCount, citations }){
    const citeBlock = (citations||[]).map(c => `PMID ${c.pmid}: ${c.title} — ${c.citation}`).join('\n');
...
The final slide should have layout "references" listing all citedPmids used across the deck, with "bullets" as formatted citation strings.`;
  ...
  /** Copilot: freeform edit/refine instruction applied to a single slide */
  ...
  /** Copilot: freeform Q&A / suggestions about the whole deck */
